...
from autoforex.core import Money, Pips, PositionSide, Units

from autoforex.snowball.config import (
    RebuildConfig,
    RebuildPriceConfig,
    SnowballConfig,
)
from autoforex.snowball.enums import CloseReason, RebuildEntryPriceMode
from autoforex.snowball.models.entries import FilledEntry, RequestedEntry
from autoforex.snowball.models.grid import Grid, Layer
from autoforex.snowball.models.identifiers import EntryId
...
class TestSnowballGridSelector:
    def test_uses_pending_stop_loss_original_as_effective_head(self) -> None:
    ...
    def test_respects_refillable_counter_limit(self) -> None:
...
class TestSnowballTakeProfitPlanner:
    def test_uses_snapshot_weighted_average_without_mutating_entries(self) -> None:
...
class TestSnowballStopLossPlanner:
    def test_buffers_stop_loss_exit_price_rebuild_mode(self) -> None:
        config = SnowballConfig(
    ...
    def test_does_not_buffer_original_entry_rebuild_mode(self) -> None:
        config = SnowballConfig(
            rebuild=RebuildConfig(
                price=RebuildPriceConfig(
